...
kable(S3, format = "latex", booktabs = TRUE, caption = "Table S2. Model form, $\\Delta$AIC, and R$^{2}$ for each climate variable as a single fixed effect in models for each C flux.", caption.short = "Table S2. Model form, $\\Delta$AIC and R$^{2}$ for each climate variable as a single fixed effect in models for each C flux", escape = F) %>%
  kable_styling(latex_options = c("scale_down", "hold_position")) %>%
...
kable(S5, format = "latex", booktabs = TRUE, caption = "Table S3. Signficance of relationships of forest C fluxes to MAT alone and in combination with MAP. $R^{2}$ values refer to the best model.",  caption.short = "Table S3. Signficance of relationships of forest C fluxes to MAT alone and in combination with MAP.", escape = F)%>%
...
kable(S3, format = "latex", booktabs = TRUE, caption = "Table S5. Best (lowest $\\Delta$AIC) single-climate variable models by C flux.", caption.short = "Table S5. Best models by carbon flux", escape = F) %>%
  kable_styling(latex_options = c("scale_down", "hold_position")) %>%
...
kable(S4, format = "latex", booktabs = TRUE, caption = "Table S6. Pairwise comparisons of correlations with climate variables between C fluxes, with all analyses conducted on a set of sites common to each pair.", escape = F) %>%
  kable_styling(latex_options = c("scale_down", "hold_position")) %>%
...
\newpage
```{r echo=FALSE, fig.cap = "Figure S1: Maps showing distribution of samples for the nine forest C fluxes analyzed", out.height ='95%', fig.pos='H'}

  knitr::include_graphics("tables_figures/distribution_all_samples.png")
```
\newpage 
\blandscape
```{r echo=FALSE, fig.align = "center", fig.cap = "Figure S2: Scatterplots and Pearson's R values for relationships among climate variables and latitude", out.height='95%', fig.pos='H'}
...
```
Regressions test variation in allocation to component fluxes for three relationships: (1) $GPP = NPP + R_{auto}$, (2) $NPP = ANPP + BNPP$, and (3) $ANPP = ANPP_{foliage} + ANPP_{stem}$. We interpret analysis of variation in $NPP:R_{auto}$ in the context of CUE (GPP/NPP), as variation in CUE will be directly related to variation in $NPP:R_{auto}$.
