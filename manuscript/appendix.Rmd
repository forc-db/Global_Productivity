...
kable(S3, format = "latex", booktabs = TRUE, caption = "Table S2. Model form, delta AIC, and R2 for each climate variables as a single fixed effect in models for each C flux. Model forms include first-order linear (Lin), second-order polynomial (Poly), and logarithmic (Log).") %>%
  kable_styling(latex_options = c("scale_down", "hold_position")) %>%
...
kable(S3, format = "latex", booktabs = TRUE, caption = "Table S5. Best models by carbon flux. Models where delta AIC </= 2 are presented", escape = F) %>%
  kable_styling(latex_options = c("scale_down", "hold_position")) %>%
...
\blandscape
```{r echo=FALSE, fig.align = "center", fig.cap = "Figure S2: Correlations among latitude and climate variables", out.height='95%', fig.pos='H'}
