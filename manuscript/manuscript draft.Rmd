...
|--------------|-------------------|-------------|------------------------|-------|-------|
| GPP | Annual gross primary production; annual uptake of carbon dioxide by an ecosystem | NA | Flux partitioning of eddy covariance | 254 | 91 |
| NPP | Annual net primary production; the component of GPP that is stored in plant tissue; GPP minus ecosystem respiration | Foliage, branch, stem, coarse root, fine root and optionally understory | Direct measurement of annual increments of components | 116 | 85 |
| ANPP | Aboveground net primary production | Foliage, stem, and optionally branch | Direct measurement of annual increments of components | 259 | 170 |
| ANPP~foliage~ | Net primary production of foliage | Foliage | Direct measurement of litterfall, correcting for changes in leaf biomass when measured | 100 | 96 |
| ANPP~woodystem~ | Net primary production of woody stems | Woody stems | Direct measurement of stem growth increment | 264 | 252 |
| BNPP~root~ | Belowground net primary production | Coarse and fine roots | Direct measurement of one or more of: fine root turnover, soil cores, root ingrowth cores, minirhizotrons; indirect estimates of coarse roots using allometries based on aboveground stem increment measures | 105 | 90 |
| BNPP~rootfine~ | Net primary production of fine roots | Fine roots | Direct measurement of one or more of: minirhizotrons, fine root turnover, soil cores, root ingrowth cores | 96 | 88 |
...
| R~root~ | Annual root respiration | Coarse and fine roots | Measurement of root gas exchange | 57 | 38 |
...
To investigate the potential interactive effects of climate variables on carbon fluxes, multivariate models were also specified. Climate variables which were non-significant or poor predictors in the models with a single climate term, or which were strongly correlated with another climate term, were dropped, leaving a subset of 7 climate variables: mean annual temperature; temperature seasonality; mean annual precipitation; precipitation seasonality; potential evapotranspiration; vapour pressure deficit; and solar radiation. For each possible pairing of climate variables, two models were specified: a model with the two climate variables showing an additive effect; and a model with the two climate variables showing an interactive effect. In addition a null model, and models including each climate term as a individual (univariate) linear term were specified. As before, altitude was included as a fixed effect, and plot nested within geographic area as a random effect. AIC values were calculated for the models and used to select the best model. R-squared values were calculated for the best model. Slope coefficients were calculated for each of the terms, and for the interactive term if relevant. 
...
```{r echo=FALSE, out.width='100%', fig.cap = "Graphs to show primary productivity $(MgC~ha^-1~yr^-1)$ regressed against latitude. Lines of best fit are plotted according to the best model selected during analysis. All regressions are significant $(p<0.05)$.", fig.pos='H'}
  knitr::include_graphics("effect_of_latitude.png")
```
  
Latitude is a strong predictor for many of the carbon fluxes, explaining 66% of variation in GPP (n = 254, p<0.0001), 51% in NPP (n = 116, p<0.0001) and 50% in ANPP (n = 259, p<0.0001). For all fluxes, their relationship with latitude was best predicted by the linear model. 
  
Analysis of component fluxes indicates that measures of smaller component fluxes do approximately sum to measures of larger fluxes (fig. 3). Modelled estimates of GPP, generated from the sum of NPP and R auto; NPP, generated from the sum of ANPP and BNPP~root~; and ANPP, generated from the sum of ANPP~foliage~ and ANPP~woodystem~, fall completely within the confidence intervals of the regressions of field estimates of GPP, NPP and ANPP respectively.  
...
Looking across all productivity components, the climate variables which explain the highest amount of variation in carbon fluxes are temperature seasonality, mean annual temperature, annual temperature range, and annual frost days. Vapour pressure deficit came out as a strong predictor for ANPP~woodystem~, and potential evapotranspiration was a good predictor for BNPP~root~ and ANPP~woodystem~. 
...
**Temperature seasonality and annual temperature range.** These two climate variables are strongly correlated, and so only temperature seasonality is presented in figure 4. Productivity is highest where temperature seasonality = 0, and at an annual temperature range of 15$^\circ$C or lower. GPP, NPP, ANPP, and R~root~ show a polynomial relationship with both variables, and BNPP~root~ shows a polynomial relationship with annual temperature range. As seasonality increases, these variables show a more rapid decrease in productivity. ANPP~foliage~ and ANPP~woodystem~ show a linear relationship with both variables.  
**Mean annual precipitation.** Despite being a poor predictor of productivity, the results indicate that, for the majority of fluxes, productivity increases with precipitation, up until a saturation point at between 3000 and 4000mm annual precipitation, above which productivity starts to decrease. The notable exception to this is GPP, which shows a significant (p<0.0001) linear increase with precipitation up to measures of nearly 6000mm annually, although the explanatory power of this model was low (r-squared = 0.15).  
**Potential evapotranspiration.** All fluxes, with the exception of BNPP~rootfine~ and R~root~ show a strong increase with potential evapotranspiration at values below 1000mm. Between 1200 and 1700mm productivity peaks, and begins to decrease at values above 1800mm.  
  
Including a second climate term in the models tended to have only a small effect on the explanatory power of the models. For ANPP~woody stem~ and R~auto~, including two climate terms did not improve the explantory power of the models: mean annual temperature alone was the best climate predictor for both of these fluxes. For all the remaining carbon fluxes, including a second climate term did improve the explanatory power of the models. ANPP and ANPP~foliage~ were best explained by an additive model, in both cases including mean annual temperature and potential evapotranspiration as the climate terms. GPP, NPP, BNPP, BNPP~fine root~, and R~root~ were all best explained by multivariate models which included an interaction term between the two climate terms. The best models for GPP, NPP, and R~root~ all included a temperature term and a precipitation term, indicating that although precipitation isn't a strong predictor for these variables, the interaction between temperature and water availability is important. 

  
```{r eval = TRUE, echo=FALSE, warning=FALSE}
library(knitr)
library(kableExtra)
multivariate_models <- read.csv("C:/Users/banburymorganr/Dropbox (Smithsonian)/GitHub/Global_Productivity/manuscript/multivariate_models.csv", stringsAsFactors = FALSE, check.names = FALSE)
rownames(multivariate_models) <- c("GPP", "NPP", "ANPP", "ANPP\\textsubscript{foliage}", "ANPP\\textsubscript{woody stem}", "BNPP", "BNPP\\textsubscript{fine root}","R\\textsubscript{auto}", "R\\textsubscript{root}")
kable(multivariate_models, format = "latex", booktabs = TRUE, caption = "Multivariate model output summary. For each carbon flux, correlation coefficients are presented for the two climate variables which explain the most variation when included in a multivariate model. If the model includes an interactive effect, correlation coefficients for the interaction are presented. For carbon fluxes that were best predicted by a single climate term, correlation coefficients are presented for that term only.  ", escape = FALSE, row.names = TRUE) %>%
  kable_styling(latex_options = c("scale_down", "hold_position")) %>%
  column_spec(1:10, width = "2cm") %>%
  add_header_above(c(" ", " ", " ", "Correlation coefficients of climate variables" = 7, " "))
...
| Variable | Latitude | Mean annual temperature | Temperature seasonality | Potential evapotranspiration | Multivariate model |
|----------|----------|----------|----------|--------------|----------|
| R~auto~ | 0.64 | 0.83 | 0.54 | 0.63 | NA |
| GPP | 0.66 | 0.61 | 0.75 | 0.40 | 0.73 |
| NPP | 0.51 | 0.49 | 0.52 | 0.34 | 0.57 |
| ANPP | 0.50 | 0.45 | 0.47 | 0.32 | 0.50 |
| ANPP~foliage~ | 0.55 | 0.63 | 0.54 | 0.40 | 0.66 |
| ANPP~woodystem~ | 0.25 | 0.23 | 0.20 | 0.26 | NA |
| BNPP~root~ | 0.31 | 0.26 | 0.31 | 0.32 | 0.38 |
| BNPP~rootfine~ | 0.14 | 0.11 | 0.16 | 0.11 | 0.19 |
| R~root~ | 0.25 | 0.21 | 0.29 | 0.04* | 0.43 |
  
Table: R-squared values presented for latitude, three climate variables, and the best multivariate model for each carbon flux. Values marked * are not significant. 
  
R-squared values are generally highest in the major fluxes, and decrease in subsidiary fluxes. Of the major fluxes, R~auto~ and GPP are the most strongly explained by latitude and climate. Mean annual temperature explains 83% of variation in R~auto~, while temperature seasonality explains 75% of variation in GPP. The proportion of variation explained by climate and latitude decreases in NPP and ANPP. The climatic variables with the strongest explanatory power explain around 50% of the variation in these fluxes. Of the major fluxes, BNPP~root~ is the least well explained by climate and latitude, with climate explaining at most 32% of variation.  
  
With the exception of ANPP~foliage~, the proportion of variation explained by climate and latitude in subsidiary fluxes is much lower. Climate explains at most 26% of variation in ANPP~woodystem~, 17% in BNPP~rootfine~, and 32% in R~root~. In contrast, climate strongly explains variation in ANPP~foliage~, with mean annual temperature explaining 63% of variation.

This pattern is also seen in the r-squared values for multivariate models. 
