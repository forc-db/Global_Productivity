...
In this analysis we use a comprehensive global database, standardised to account for differences in methodology across studies, and stand age and level of disturbance across sites. The database provides us with an unprecedented amount of data, representing all (?) global forest ecosystems and all significant forest carbon fluxes. This breadth of data allows us to comprehensively explore the relationships between climate and productivity on a global scale. 

Our results show that productivity decreases linearly with latitude. Climate explains a significant proportion of variation in all carbon fluxes, with temperature variables being the best predictors of productivity. While other climate variables are significant predictors of productivity, none of them improve on the explanatory power of temperature-related variables. Water availability is an important factor in explaining productivity on a global scale: we find a positive influence of precipitation at low MAP, with saturation at higher levels of MAP. There is a significant interaction between MAT and MAP. 
...
This studies considers the relationship between productivity and climate on a coarse scale. Because of the nature of the ForC database, we were unable to access site-level MAT and MAP data for a number of sites, and lacked site-level data for other climate variables for all sites. As a result the climate data we used was primarily multi-year averages of annual climate measures from global databases. While the quality of these data is very high, we cannot expect that it captures detailed local variation in climate. Although we find that MAT is the strongest predictor for climate, part of this effect may be a result of the higher-quality stand-level dataset available for MAT. With high-quality stand-level data for other climate variables we might find stronger predictors. Similarly, with accurate stand-level measurements of growing season length, we would expect to record a stronger correlation between growing season length and productivity. 

Furthermore, we expect that forest stand productivity varies on a seasonal and annual scale with seasonal and annual variation in climate. By taking long-term averages both in climate and in forest productivity estimates, we expect that much of this variation will be averaged out, and the strong local effects of climate on productivity will be masked.

By testing a wide selection of carbon fluxes, we have been able to explore the relationships between fluxes and their responses to productivity. We see that:
- All fluxes show similar responses to climate drivers
- Allocation to fluxes does not vary with climate on a global scale. This suggests that the pressures that require individuals to shift allocation between fluxes occur on much more local scales. We expect that allocation varies with factors such as altitude, stand age, disturbance regime, micro-climate, and is likely to show seasonal responses to climate variables, such as showing short-term responses to water availability.
- Climate more strongly explains variation in the major fluxes, with lower levels of variation explained in subsidiary fluxes. This indicates that factors other than climate influence subsidiary fluxes. This links to the results we find for allocation: although climate has a significant effect on these fluxes, there are a number of other factors which can influence the way that carbon is allocated to them, which masks the effect of climate to some extent. 

Limitations: 
- We have only considered mature forests; there is a lot more work to go towards understanding the effect of stand age on productivity. In part, understanding these patterns will be reliant on acquiring more data at all stand ages.
- We have standardised data as far as possible; however there are still challenges with ensuring data quality is as high as possible. For example, there are inherent challenges with measuring stand-level productivity, particularly for certain components. Standardising BNPP in particular is challenging, as it is often sampled to insufficient depth/extrapolated from measures of ANPP. This means that, although we have been careful to control data selection within the database, we cannot entirely control the quality of data going into the database. As a result, our results are likely to be affected by this.
- As above, we are looking at climate data at a coarse scale; these are generalisations about large-scale patterns. Stand-level studies will help to improve our understanding of the effect of climate on productivity on smaller spatial and temporal scales.
