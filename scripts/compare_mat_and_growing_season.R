...
for (response.v in response.variables){
  ...
  mod.mat <- lmer(scale(mean) ~ poly(mat, 1, raw = T) + (1|geographic.area/plot.name), data = df, REML = F)
  mod.growing <- lmer(scale(mean) ~ poly(length_growing_season, 1, raw = T) + (1|geographic.area/plot.name), data = df, REML = F)
  
  aictab <- aictab(list(mod = mod, mod.mat = mod.mat, mod.growing = mod.growing), sort = T)
...
write.csv(all.aictab, file = "C:/Users/becky/Dropbox (Smithsonian)/GitHub/Global_Productivity/results/tables/best_model_outputs/AIC_table_mat_growing_season.csv", row.names = F)