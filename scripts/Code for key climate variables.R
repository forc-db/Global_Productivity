...
fixed.variables <- c("AnnualMeanTemp", "Isothermality", "MinTColdestMonth", "TempRangeAnnual", "PreSeasonality", "PreColdQ", "AnnualFrostDays")

## prepare results table

all.results <- NULL

for (n in 1:length(response.variables.groups))
 ...
 ages.to.keep <- ForC_simplified$stand.age >= 100 & !is.na(ForC_simplified$stand.age)
  ...
  for(fixed.v in fixed.variables){
    ...
    for (n in 1:length(response.variables.groups)) {
      ...
      if(n == 1) response.variables <- c("GPP", "NPP", "ANPP")
      if(n == 2) response.variables <- c("ANPP_foliage", "ANPP_woody", "BNPP_root")
      
      tiff(file = paste0("C:/Users/banburymorganr/Dropbox (Smithsonian)/GitHub/Global_Productivity/results/figures/test/refining_model/Effect_of_", fixed.v, "_MATURE_only_", n, ".tiff"), width = 2255, height = 2000, units = "px", res = 300)
      ...
      for (response.v in response.variables){ #, "ANPP_stem"
        ...
        if(nrow(df) > 30 & ifelse(!categorical | (categorical & length(unique(df[, fixed.v])) >1), TRUE, FALSE)){
          ...
          results <- data.frame(response = response.v, fixed = fixed.v, random = "geographic.area/plot.name", equation = equation, significant = significant.effect, p.value = significance, sample.size = sample.size)
