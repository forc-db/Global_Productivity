...
for(response.variables in response.variables.groups){
  ...
  for (age in ages){
    ...
    for(fixed.v in fixed.variables){
      ...
      tiff(file = paste0("C:/Users/banburymorganr/Dropbox (Smithsonian)/GitHub/Global_Productivity/results/figures/test/scaled_model/linear/Effect_of_", fixed.v, "_MATURE_only_", age, "_", n, "_linear.tiff"), width = 2255, height = 2000, units = "px", res = 300)
      ...
      for(response.v in response.variables){
        ...
        subset <- ForC_simplified[rows.with.response & ages.to.keep & fixed.no.na, ]
        
        range <- range(scale(subset$mean))
        
        all.ranges <- as.data.frame(rbind(range, all.ranges))
      ...
      for (response.v in response.variables){
        ...
        ylim <- c(min(all.ranges$V1), max(all.ranges$V2))
...
for(response.variables in response.variables.groups){
  ...
  for (age in ages){
    ...
    for(fixed.v in fixed.variables){
      ...
      tiff(file = paste0("C:/Users/banburymorganr/Dropbox (Smithsonian)/GitHub/Global_Productivity/results/figures/test/scaled_model/polynomial/Effect_of_", fixed.v, "_MATURE_only_", age, "_", n, "_polynomial.tiff"), width = 2255, height = 2000, units = "px", res = 300)
      ...
      for (response.v in response.variables){
        ...
        mod <-  lmer(scale(mean) ~ 1 + (1|geographic.area/plot.name), data = df, weights = weight)
        mod.full <- lmer(scale(mean) ~ poly(fixed, 2) + (1|geographic.area/plot.name), data = df, weights = weight)
        significant.effect <- anova(mod, mod.full)$"Pr(>Chisq)"[2] < 0.05
        significance <- anova(mod, mod.full)$"Pr(>Chisq)"[2]
        sample.size <- length(df$mean)
        ...
        if(first.plot) plot(scale(mean) ~ fixed, data = df, xlab = "", ylab = "", col = response.v.color, xaxt = "n", yaxt = "n", ylim = ylim)
...
for (age in ages){
  ...
  for(fixed.v in fixed.variables){
    ...
    tiff(file = paste0("C:/Users/banburymorganr/Dropbox (Smithsonian)/GitHub/Global_Productivity/results/figures/test/scaled_model/polynomial/Effect_of_", fixed.v, "_MATURE_only_poly_all.tiff"), width = 2255, height = 2000, units = "px", res = 300)
