...
ForC_simplified <- ForC_simplified[ForC_simplified$variable.name %in% c("NPP_1", "NPP_2", "NPP_3", "NPP_4", "NPP_5", "NPP_0","GPP", "R_auto"),]

GPP <- ForC_simplified[ForC_simplified$variable.name %in% c("GPP"),]
...
R_auto <- ForC_simplified[ForC_simplified$variable.name %in% c("R_auto"),]
...
png(file = "C:/Users/becky/Dropbox (Smithsonian)/GitHub/Global_Productivity/results/figures/final_figures/ratio_regressions/log ratios/NPP_Rauto.png", width = 3000, height = 2000, units = "px", res = 300)
...
mtext(text = "Log(NPP/Autotrophic respiration)", side = 2, outer = T, line = 1)
