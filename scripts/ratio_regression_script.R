  ...
  # for (j in seq(along = set2)){
    # if (i == j){
      ...
      for(fixed.v in fixed.variables){
        ...
        mod.poly <- lmer(ratio ~ poly(fixed, 2, raw = T) + masl + (1|geographic.area/plot.name), data = df, REML = F)
        ...
        if (best.model == "mod.poly") mod.full <- lmer(ratio ~ poly(fixed, 2, raw = T) + masl + (1|geographic.area/plot.name), data = df, REML = F)
        ...
        if (best.model == "mod.poly") mod.full <- lmer(ratio ~ poly(fixed, 2, raw = T) + masl + (1|geographic.area/plot.name), data = df, REML = T)
